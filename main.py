    ...
    for m in mut_range:
        ...
        if w_mut < chprob:
            ...
            for i in r_range:
                ...
                rct_ind = map(int, rct_ind)
                prd_ind = [s.replace('S', '') for s in prd[i]]
                prd_ind = map(int, prd_ind)
            ...
            antStr = ng.genAntimonyScript(flt_id, bnd_id, flt_id, bnd_id, 
        ...
        if w_mut >= chprob:
            ...
            while len(stt[1]) != realNumFloating or len(stt[2]) != realNumBoundary:
                ...
                if rt == ng.TReactionType.UNIUNI:
                    ...
                    while len(set(all_rct) & set(all_prd)) > 0:
                        rct_id = np.random.choice(np.arange(ns), size=1)
                        prd_id = np.random.choice(np.delete(np.arange(ns), 
                                                rct_id), size=2, replace=True)
                        all_rct = [i for i, x in enumerate(rct) if x == ['S'+str(rct_id[0])]]
                        all_prd = [i for i, x in enumerate(prd) if x == ('S'+str(prd_id[0]),
                                'S'+str(prd_id[1])) or x == ('S'+str(prd_id[1]), 'S'+str(prd_id[0]))]
            ...
            antStr = ng.genAntimonyScript(realFloatingIds, realBoundaryIds, 
                    stt[1], stt[2], reactionList, boundary_init=realBoundaryVal)
        
        try:
            r = te.loada(antStr)
            if ((realFloatingIds == np.sort(r.getFloatingSpeciesIds())).all() 
            and (realBoundaryIds == np.sort(r.getBoundarySpeciesIds())).all()):
...
def polish(listantStr, listdist):
    ...
    for m in psize_range:
        ...
        antStr = ng.genAntimonyScript(flt_id, bnd_id, flt_id_s, bnd_id_s, 
                                      reactionList, boundary_init=realBoundaryVal)
        ...
        try:
            ...
            and (realBoundaryIds == np.sort(r.getBoundarySpeciesIds())).all()):
                ...
                if np.any(SS_i < 1E-5) or np.any(SS_i > 1e5):
                    ...
                    dist_i = ((np.sqrt(np.sum(np.square(realFlux - F_i))) 
...
def initialize():
    ...
    while numGoodModels < ens_size:
        ...
        if (numIter/10000.) == (numIter/10000):
...
# Settings ===================================================================
    ...
    pass_size = ens_size/10 # Size of models passed on the next generation 
                            # without mutation
    mut_size = ens_size/2 # Size of models selected and mutated
    n_polish = 200 # Number of polishing steps to run
    polish_size = 100 # Size of polishing population
    nrnd_size = pass_size+mut_size
    rnd_size = ens_size-pass_size-mut_size
    
    MKP = 0.3 # Probability of changing rate constants on mutation. 
              # Probability of changing reaction is 1 - MKP
    rateStep = 0.1 # Stepsize for mutating rate constants. 
                   # Actual stepsize is rateConstant*rateStep
    ...
    r_seed = 123123
    r_roulette = False # Flag for using random roulette or best of pair for 
                       # selection process
    
    PLOT = True # Flag for plotting
    SAVE = False # Flag for saving the plot
...
# Settings end ===============================================================
    ...
    ens_dist, ens_model = initialize()
    
    dist_top = ens_dist[np.argsort(ens_dist)]
    model_top = ens_model[np.argsort(ens_dist)]
    ...
    for n in n_range:
        if r_roulette:
            ...
            if mut_size > (ens_size)/2.:
                mut_pair = np.random.choice(np.arange(ens_size), 
                                            size=(mut_size, 2), replace=True)
            else:
                mut_pair = np.random.choice(np.arange(ens_size), 
                                            size=(mut_size, 2), replace=False)
            mut_ind = np.sort(mut_pair[np.arange(mut_size), 
                                       np.argmin(dist_top[mut_pair], axis=1)])

        ens_model[:pass_size] = model_top[:pass_size]
        ens_dist[:pass_size] = dist_top[:pass_size]
        
        eval_dist, eval_model = mutate_and_evaluate(model_top[mut_ind], 
                                                    dist_top[mut_ind], mut_ind)
    ...
    for p in p_range:
        mut_pair = np.random.choice(np.arange(polish_size), 
                                    size=(polish_size-1, 2), replace=True)
        mut_ind = np.sort(mut_pair[np.arange(polish_size-1), 
                                   np.argmin(pdist_top[mut_pair], axis=1)])
    ...
    if PLOT:
        ...
        if SAVE:
            plt.savefig(os.path.join('./convergence_' + model_type + '.pdf'), 
                        bbox_inches='tight')
        plt.show()
        ...
        plt.yticks(fontsize=15)
        if SAVE:
            plt.savefig(os.path.join('./convergence_p_' + model_type + '.pdf'), 
                        bbox_inches='tight')
        plt.show()
        ...
        plt.yticks(fontsize=15)
        if SAVE:
            plt.savefig(os.path.join('./average_residual_' + model_type + '.pdf'), 
                        bbox_inches='tight')
        plt.show()
        ...
        plt.yticks(fontsize=15)
        if SAVE:
            plt.savefig(os.path.join('./average_residual_p_' + model_type + '.pdf'), 
                        bbox_inches='tight')
        plt.show()
